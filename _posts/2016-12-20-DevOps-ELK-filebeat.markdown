...
filebeat用來在各ap主機收集指定路徑的紀錄檔，再發送到logstash，以下是建置步驟:

1. 新增filebeat資料夾，並放置配置檔案：

		$ sudo mkdir -p /home/fusion-ap/filebeat && sudo mkdir -p /home/fusion-ap/filebeat-registry
		$ sudo vi /home/fusion-ap/filebeat/filebeat.yml
	...
	以下為filebeat.yml內容：
		...
		filebeat:
		  ...
		  prospectors:
		    ...
		    -
		      paths:
		       - /srv/fusion-ap/asset/logs/*.log
		      document_type: tomcat_log
		    -
		      paths:
		       - /srv/fusion-ap/asset-rest/logs/*.log
		      document_type: tomcat_log

		    -
		      paths:
		       - /srv/fusion-ap/cust-visit/logs/*.log
		      document_type: tomcat_log

		    -
		      paths:
		       - /srv/fusion-ap/cust-visit-rest/logs/*.log
		      document_type: tomcat_log

		    -
		      paths:
		       - /srv/fusion-ap/provident/logs/*.log
		      document_type: tomcat_log

		    -
		      paths:
		       - /srv/fusion-ap/financial/logs/*.log
...
2. create run_filebeat.sh && run it !

		$ cd
		$ vi run_filebeat.sh

	以下是run_filebeat.sh內容：

		#!/bin/bash
		docker rm -f filebeat
		docker run --name filebeat -itd \
		--restart=always \
		-v /home/fusion-ap/filebeat:/etc/filebeat \
		-v /home/fusion-ap/filebeat-registry:/etc/registry \
